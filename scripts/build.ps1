...
switch ($Profile) {
    "dev"     { $BuildType = "Debug"; $EnableTests = "ON" }
    "san"     { $BuildType = "Debug"; $EnableAsan = "ON"; $EnableTests = "ON" }
    "release" { $BuildType = "Release"; $EnableTests = "OFF" }
    "ci"      { $BuildType = "Debug"; $EnableAsan = "ON"; $EnableTests = "ON" }
    "full"    { $BuildType = "Release"; $EnableHttp = "ON"; $EnableTests = "ON" }
}

if ($Debug)          { $BuildType = "Debug" }
if ($Release)        { $BuildType = "Release" }
if ($RelWithDebInfo) { $BuildType = "RelWithDebInfo" }
if ($MinSizeRel)     { $BuildType = "MinSizeRel" }
if ($Tests)          { $EnableTests = "ON" }
if ($NoTests)        { $EnableTests = "OFF" }
if ($Http)           { $EnableHttp = "ON" }
if ($Otel)           { $EnableOtel = "ON" }
if ($Tui)            { $EnableTui = "ON" }
if ($Asan)           { $EnableAsan = "ON" }

# Build directory
if (-not $BuildDir) {
    $suffix = $BuildType.ToLower()
    if ($EnableAsan -eq "ON") { $suffix += "-asan" }
...
if ($Compiler) { CfgLine "Compiler" $Compiler "yellow" }

$features = @()
if ($EnableTests -eq "ON") { $features += "tests" }
if ($EnableHttp -eq "ON")  { $features += "http" }
if ($EnableOtel -eq "ON")  { $features += "otel" }
if ($EnableTui -eq "ON")   { $features += "tui" }
CfgLine "Features" (($features -join " ") + $(if ($features.Count -eq 0) { "none" } else { "" }))

if ($EnableAsan -eq "ON") { CfgLine "Sanitizers" "ASan" "red" }
else                      { CfgLine "Sanitizers" "none" "dim" }

if ($Profile) { CfgLine "Profile" $Profile "magenta" }
Write-Host ""
...
if ($ninja) { $CmakeArgs += @("-G", "Ninja") }

switch ($Compiler) {
    "clang-cl" { $CmakeArgs += @("-T", "ClangCL") }
    "clang"    { $CmakeArgs += @("-DCMAKE_C_COMPILER=clang", "-DCMAKE_CXX_COMPILER=clang++") }
    "gcc"      { $CmakeArgs += @("-DCMAKE_C_COMPILER=gcc", "-DCMAKE_CXX_COMPILER=g++") }
}
...
if ($Prefix) { $CmakeArgs += "-DCMAKE_INSTALL_PREFIX=$Prefix" }
...
if ($DryRun) {
    ...
    Write-Host ""
    if ($Clean) { Write-Host "  Remove-Item -Recurse -Force `"$BuildDir`"" }
    Write-Host "  cmake $($CmakeArgs -join ' ')"
    if (-not $ConfigureOnly) { Write-Host "  cmake --build `"$BuildDir`" --config $BuildType -j $Jobs" }
    if ($Install) { Write-Host "  cmake --install `"$BuildDir`" --config $BuildType" }
    Write-Host ""; exit 0
}
...
if (-not $clCmd) {
    ...
    if (-not $vswhere) { Die "Cannot find vswhere.exe. Install Visual Studio Build Tools." }

    $vsPath = & $vswhere -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath 2>$null
    if (-not $vsPath) { $vsPath = & $vswhere -latest -products * -property installationPath 2>$null }
    if (-not $vsPath) { Die "No Visual Studio installation found." }

    $vcvarsall = Join-Path $vsPath "VC\Auxiliary\Build\vcvarsall.bat"
    if (-not (Test-Path $vcvarsall)) { Die "vcvarsall.bat not found at: $vcvarsall" }

    $arch = if ([Environment]::Is64BitOperatingSystem) { "x64" } else { "x86" }
    Log "Loading MSVC environment from: $vsPath ($arch)..."

    $envDump = cmd /c "`"$vcvarsall`" $arch >nul 2>&1 && set" 2>$null
    if ($LASTEXITCODE -ne 0) { Die "vcvarsall.bat failed." }
...
if ($Clean) {
    if (Test-Path $BuildDir) {
        ...
        Ok "Clean complete"
    } else { Warn "Build directory doesn't exist, nothing to clean" }
}
...
if ($LASTEXITCODE -ne 0) { Die "CMake configuration failed (exit code $LASTEXITCODE)" }

$configTime = [math]::Round(((Get-Date) - $configStart).TotalSeconds, 1)
Ok "Configured in ${configTime}s"

if ($ConfigureOnly) { Write-Host ""; Ok "Configure-only mode -- skipping build"; exit 0 }
...
& cmake @buildArgs 2>&1 | ForEach-Object {
    $line = $_
    if ($line -match "error[:\s]|Error")         { Write-Host "  $esc[31m$line$esc[0m" }
    elseif ($line -match "warning[:\s]")         { Write-Host "  $esc[33m$line$esc[0m" }
    elseif ($line -match "Building|Linking|Built target") { Write-Host "  $esc[32m$line$esc[0m" }
    else                                          { Write-Host "  $esc[2m$line$esc[0m" }
}
if ($LASTEXITCODE -ne 0) { Die "Build failed (exit code $LASTEXITCODE)" }
...
if ($Install) {
    Hdr "Install"
    $installPrefix = if ($Prefix) { $Prefix } else { "C:\Program Files\Kairos" }
...
if (-not (Test-Path $kairosBin)) { $kairosBin = Join-Path $BuildDir "kairos.exe" }
if (Test-Path $kairosBin) { CfgLine "Binary" $kairosBin }
